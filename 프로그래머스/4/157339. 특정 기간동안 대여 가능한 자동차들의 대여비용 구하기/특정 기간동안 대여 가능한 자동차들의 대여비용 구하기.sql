...
# WHERE !((DATE_FORMAT(START_DATE,'%y-%m-%d') > '22-11-01') AND (DATE_FORMAT(START_DATE,'%y-%m-%d') <= '22-11-30'))
...
# SELECT * FROM ANS
# SELECT CAR_ID, CAR_TYPE, FEE FROM ANS WHERE FEE >=500000 AND FEE <2000000
# ORDER BY FEE DESC, CAR_TYPE ASC, CAR_ID DESC

# (SELECT DP.CAR_TYPE,DISCOUNT_RATE FROM CAR_RENTAL_COMPANY_DISCOUNT_PLAN AS DP WHERE DURATION_TYPE = '30일 이상') USING (CAR_TYPE)
# WHERE CAR_ID NOT IN (SELECT CAR_ID FROM HISTORY) AND 
# CAR_TYPE IN ('세단','SUV')
